...
use lospiratas\islands\generator\WorldGenerator;
use lospiratas\islands\IslandManager;
...
class Islands extends PluginBase implements Listener{
	...
	public function onEnable(){
		...
		$this->islandManager = new IslandManager($this, $this->world, $islandSize, $spawnCurl);
	...
	public function onMove(PlayerMoveEvent $event){
		...
		$island = $this->islandManager->getIslandFromPos($player);

		if(!isset($this->inSpawn[$player->getName()])){
			$this->inSpawn[$player->getName()] = false;
		}

		// last island that player entered
		if(!isset($this->playerLastIsland[$player->getName()])){
			$this->playerLastIsland[$player->getName()] = $island;
		}

		if($this->islandManager->isInSpawn($player)){

			// if player isnt already in spawn, send message and set player in spawn
			if(!$this->inSpawn[$player->getName()]){
				$player->addTitle("\n", "§aVocê entrou no spawn.");
				$this->inSpawn[$player->getName()] = true;
			}

		}else{
			$this->inSpawn[$player->getName()] = false;

			// if island isnt last island player entered = player is in new island
			if($this->playerLastIsland[$player->getName()] != $island){
				$player->addTitle("\n", "§aIlha §f$island");
				$this->playerLastIsland[$player->getName()] = $island;
			}
		}
